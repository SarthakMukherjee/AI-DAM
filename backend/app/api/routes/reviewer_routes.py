...
def reject_asset(asset_id: str, request: Request, body: ReviewRequest, db: Session = Depends(get_db), current_user: User = Depends(require_reviewer)):
    ...
    log_audit_event(
        ...
        field_name="status",
        old_value=old_status,
        new_value=f"rejected (Category: {body.rejection_category}, Reason: {body.reason})",
        ip_address=request.client.host if request.client else None
    )
    db.commit()
...
def publish_asset(asset_id: str, body: PublishRequest, request: Request, db: Session = Depends(get_db), current_user: User = Depends(require_reviewer)):
    asset = db.query(Asset).filter(Asset.id == asset_id).first()
    if not asset: raise HTTPException(status_code=404, detail="Asset not found")
    check_reviewer_scope(asset, current_user)
...
def restrict_asset(asset_id: str, body: RestrictRequest, request: Request, db: Session = Depends(get_db), current_user: User = Depends(require_reviewer)):
    asset = db.query(Asset).filter(Asset.id == asset_id).first()
    if not asset: raise HTTPException(status_code=404, detail="Asset not found")
    check_reviewer_scope(asset, current_user)
    ...
    log_audit_event(
        ...
        field_name="status",
        old_value=old_status,
        new_value=f"restricted (Roles: {body.restricted_to_roles})",
        ip_address=request.client.host if request.client else None
    )
    db.commit()
...
def unrestrict_asset(asset_id: str, request: Request, db: Session = Depends(get_db), current_user: User = Depends(require_reviewer)):
    asset = db.query(Asset).filter(Asset.id == asset_id).first()
    if not asset: raise HTTPException(status_code=404, detail="Asset not found")
    check_reviewer_scope(asset, current_user)
